...
### 1.1 Yaw reaction torque: throttle-vs-RPM ambiguity
...
The applied yaw torque is linear in `throttle`:
`Tᵢ = max_force · throttle`, `τ_yaw = κR · Σ sᵢ · Tᵢ`.

Real props have $T \propto \Omega^2$ and $Q \propto \Omega^2$, so $Q/T$ is
constant — the coefficient form is defensible. **But** it is only
defensible if `throttle ∈ [0,1]` represents *thrust fraction*, not RPM
fraction.  The current `PowerRamp` spool-up behaviour reads like RPM
dynamics (first-order lag), while the thrust output reads like thrust
fraction.  Pick one interpretation and document it.

### 1.2 Spool-up filter applied to thrust, not RPM
**File:** [DroneThrusterScript.gd](../Drones/Scripts/DroneThrusterScript.gd) (`_physics_process` ramp)
**Impact:** medium · **Effort:** low

The first-order LPF is applied to `throttle`, then thrust is
`max_force · throttle`.  Real motors: *RPM* lags the command, and
thrust follows as $\Omega^2$.  For a physically correct output, ramp an
internal `rpm_frac`, and emit `thrust = max_force · rpm_frac²`.  The
yaw-drag coupling will still be consistent as long as it reads the same
post-ramp value.

### 1.3 Ground-effect height uses slant distance, not vertical
...
Currently `h = global_position.distance_to(collision_point)`.  When the
drone tilts, the downward ray is no longer vertical and slant distance
> vertical distance → ground effect is *under*-estimated during tilted
hover / landing approaches.

Fix: use the world-Y component.
```gdscript
var h: float = global_position.y - _ground_ray.get_collision_point().y
```
(Or the dot of the offset onto world-up, if tilted terrain support
matters.)

### 1.4 Cheeseman–Bennett domain comment is misleading
**File:** [DroneThrusterScript.gd](../Drones/Scripts/DroneThrusterScript.gd) — same function
**Impact:** cosmetic · **Effort:** trivial

The `h < R/4` clamp is correct (the formula diverges below that), but
the comment frames it as a game-feel safety cap.  Rewording would help
future readers.
...
### 2.D Thrust curve: $T \propto \Omega^2$, not linear in command
**Impact:** medium · **Effort:** low
See §1.2.
...
1. **Ground-effect vertical height** (§1.3) — trivial, real fix.
2. **RPM-vs-thrust clarification + `thrust = cmd²`** (§1.2, §2.D) — low
   effort, affects every controller downstream.
3. **Arm-length-aware mixing** (§1.7) — opens the door to non-square frames.
