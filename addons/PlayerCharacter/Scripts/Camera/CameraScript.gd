...
func _unhandled_input(event):
	...
	if event is InputEventMouseMotion:
		rotate_y(-event.relative.x * (XAxisSens * .01))
		camera.rotate_x(-event.relative.y * (YAxisSens * .01))
...
func applies(delta : float):
	...
	if playChar.stateMachine.currStateName == "Crouch":
		...
		applyCameraFOV(delta, startFOV)
		
func applyCameraFOV(delta, _startFOV=null):
	if _startFOV == null:
		_startFOV = camera.fov
		#camera.fov = lerp(camera.fov,     runFOV, fovTransitionSpeed * delta)
		#camera.fov = lerp(camera.fov, camera.fov, fovTransitionSpeed * delta)
		#camera.fov = lerp(camera.fov, camera.fov, fovTransitionSpeed * delta)
		#camera.fov = lerp(camera.fov,   startFOV, fovTransitionSpeed * delta)
	keepCameraFOV = lerp(keepCameraFOV, _startFOV, fovTransitionSpeed * delta)
	camera.fov = keepCameraFOV - zoomingValue
